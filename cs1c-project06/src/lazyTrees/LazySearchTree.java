 ...
 * 
 * Implements garbage collection to truly remove all deleted (stale) nodes from the tree.
 * 
 * @author anuva
 *
 * @param <E>
...
public class LazySearchTree<E extends Comparable< ? super E > > implements Cloneable
{
	...
	/**
	 * Garbage collection mechanism. Traverse the left and right subtrees, looking for nodes that have deleted == true. 
	 * When found, apply the real ("hard") remove().
	 * @param treeNode reference to the subtree with deleted and undeleted nodes
	 * @return reference to the node with all its deleted nodes removed.
	 */
	private LazySTNode collectGarbage(LazySTNode treeNode) {

		if (mSizeHard == 0 || treeNode == null) {
			return null;		
		}
		...
		if (treeNode != null && treeNode.deleted) {
			//System.out.println("calling remove hard for " + treeNode.data);
