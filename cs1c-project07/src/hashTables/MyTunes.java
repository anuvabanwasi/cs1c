...
public class MyTunes 
{
	...
	private void printSongCompInt(FHhashQPwFind<Integer, SongCompInt> tableOfSongIDs) {
		HashEntry<SongCompInt>[] entries = tableOfSongIDs.mArray;
		
		for(int i = 0; i < entries.length; i++) {
			HashEntry<SongCompInt> entry = entries[i];
			
			if(entry.state != tableOfSongIDs.EMPTY)
				System.out.println( i + " " + entry.data);
		}
	}
	
	private void printSongCompSong(FHhashQPwFind<String, SongsCompGenre> tableOfGenres) {
		HashEntry<SongsCompGenre>[] entries = tableOfGenres.mArray;
		
		for(int i = 0; i < entries.length; i++) {
			HashEntry<SongsCompGenre> entry = entries[i];
	...
	public void writeFindRequests(List<Integer> songIds, String filename)
	{
